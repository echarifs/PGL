...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
      ...
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
            ...
            elsifInst = new IfThenElse($elsif_cond.tree, $elsif_li.tree, elsifBranch);
            setLocation(elsifInst, $ELSE);
...
literal returns[AbstractExpr tree]
    : INT {
        try {
            ...
            setLocation($tree, $INT);
                ...
                throw new InvalidAssignValue(this, $ctx);
            }
    ...
    | fd=FLOAT {
        ...
        setLocation($tree, $fd);
         ...
         if (Float.parseFloat($fd.getText())<1.4e-45 ){
            throw new InvalidAssignValue(this, $ctx);
         }

        }
    | STRING {
        //$STRING.setText(($STRING.getText()).substring(1, ($STRING.getText()).length() - 1));
        $tree = new StringLiteral(($STRING.getText()).substring(1, ($STRING.getText()).length() - 1));
...
ident returns[AbstractIdentifier tree]
    ...
    : IDENT {
        SymbolTable st = getDecacCompiler().getSymbols(); //should i call decac compiler ?
...
class_decl returns[DeclClass tree]
    : CLASS name=ident superclass=class_extension OBRACE class_body CBRACE {
    	...
    	assert($class_body.f != null);
    	assert($class_body.mtd != null);
    	$tree = new DeclClass($name.tree, $superclass.tree, $class_body.f, $class_body.mtd);
...
class_body returns[ListDeclMethod mtd, ListDeclField f]
@init {
    $mtd = new ListDeclMethod();
    $f = new ListDeclField();
    }
    : (m=decl_method {
        $mtd.add($m.tree);
...
decl_field_set[ListDeclField f]
    : v=visibility t=type list_decl_field[f, $v.tree, $t.tree]
      SEMI
    ;
...
list_decl_field[ListDeclField f, Visibility visib, AbstractIdentifier t]
...
decl_field[Visibility visib, AbstractIdentifier tp] returns[AbstractDeclField tree]
@init {
    AbstractInitialization init;
}
    : i=ident {
      ...
      (EQUALS e=expr {
      	...
      	assert($i.tree != null);
      	$tree = new DeclField(visib, tp, $i.tree, init);
      	setLocation($tree, $i.start);
        }
    ;
...
@init {
    ...
    : type ident OPARENT params=list_params CPARENT (block {
      ...
      | ASM OPARENT code=multi_line_string CPARENT SEMI {
            ...
            methodBody = new MethodAsmBody($code.text, $code.location);
