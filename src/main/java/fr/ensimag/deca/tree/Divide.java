    ...
    public void codeGenInst(DecacCompiler compiler)
    {
        ...
        if (!compiler.getCompilerOptions().getNoCheck())
        {
            ...
            if (this.getRightOperand().getType().isInt())
                compiler.addInstruction(new CMP(new ImmediateInteger(0), tmprg));
            else
                compiler.addInstruction(new CMP(new ImmediateFloat(0), tmprg));
            compiler.addInstruction(new BEQ(new Label("ZeroDivisionError")));
	    }
        compiler.addInstruction(new LOAD(rg1, tmprg));
        if (this.getLeftOperand().getType().isInt() && this.getRightOperand().getType().isInt())
            compiler.addInstruction(new QUO(rg2, tmprg));
        else
	        compiler.addInstruction(new DIV(rg2, tmprg));
        if (rg1.isR())
